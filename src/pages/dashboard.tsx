...
import { getOrderHistoryPharmacies } from 'apiSdk/order-history-pharmacies';
import { OrderHistoryPharmacieInterface } from 'interfaces/order-history-pharmacie';
import { getOrderHistoryClients } from 'apiSdk/order-history-clients';
import { OrderHistoryClientInterface } from 'interfaces/order-history-client';

import { FormAListPage as ShowCaseList } from 'pages/form-as';

function DashboardPage() {
  const { session } = useSession();

  const formAStat = useSWR<PaginatedInterface<FormAInterface>>(
    () => `/form-as`,
    () => getFormAS({ limit: 1, offset: 0 }),
  );

  const orderHistoryPharmacieStat = useSWR<PaginatedInterface<OrderHistoryPharmacieInterface>>(
    () => `/order-history-pharmacies`,
    () => getOrderHistoryPharmacies({ limit: 1, offset: 0 }),
  );

  const orderHistoryClientStat = useSWR<PaginatedInterface<OrderHistoryClientInterface>>(
    () => `/order-history-clients`,
    () => getOrderHistoryClients({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Form AS',
      result: formAStat?.data?.totalCount,
      isLoading: formAStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Order History Pharmacies',
      result: orderHistoryPharmacieStat?.data?.totalCount,
      isLoading: orderHistoryPharmacieStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Order History Clients',
      result: orderHistoryClientStat?.data?.totalCount,
      isLoading: orderHistoryClientStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`form-as`} passHref legacyBehavior>
                  <Button
