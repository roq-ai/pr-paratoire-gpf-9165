...
import { getOrderHistoryClientById, updateOrderHistoryClientById } from 'apiSdk/order-history-clients';
...
function OrderHistoryClientEditPage() {
  ...
  const handleSubmit = async (values: OrderHistoryClientInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateOrderHistoryClientById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Order History Client',
              isCurrent: true,
            },
          ]}
        />
