...
import { getOrderCurrentById, updateOrderCurrentById } from 'apiSdk/order-currents';
...
function OrderCurrentEditPage() {
  ...
  const handleSubmit = async (values: OrderCurrentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateOrderCurrentById(id, values);
      mutate(updated);
      resetForm();
