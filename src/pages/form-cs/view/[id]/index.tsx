...
function FormCViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<FormCInterface>(
    ...
    () =>
      getFormCById(id, {
        relations: ['user', 'client_profile', 'order_current_form_c_order_idToorder_current', 'form_a'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('form_c', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/form-cs/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Controle Elements Disponible" text={data?.controle_elements_disponible} />

                <FormListItem label="Controle Pharmacotechniques" text={data?.controle_pharmacotechniques} />

                <FormListItem label="Decision Liberation" text={data?.decision_liberation} />
                ...
                {hasAccess('order_current', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Order Current Form C Order Id Toorder Current"
                    text={
                      <Link
                        as={NextLink}
                        href={`/order-currents/view/${data?.order_current_form_c_order_idToorder_current?.id}`}
                      >
                        {data?.order_current_form_c_order_idToorder_current?.form_a_pdf_path}
                      </Link>
                    }
                  />
                )}
