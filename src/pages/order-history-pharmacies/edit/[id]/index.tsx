...
import { getOrderHistoryPharmacieById, updateOrderHistoryPharmacieById } from 'apiSdk/order-history-pharmacies';
...
function OrderHistoryPharmacieEditPage() {
  ...
  const handleSubmit = async (values: OrderHistoryPharmacieInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateOrderHistoryPharmacieById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Order History Pharmacie',
              isCurrent: true,
            },
          ]}
        />
      ...
      <Box rounded="md">
        <Box mb={4}>
          <Text as="h1" fontSize={{ base: '1.5rem', md: '1.875rem' }} fontWeight="bold" color="base.content">
            Update Order History Pharmacie
          </Text>
        </Box>
        {(error || formError) && (
          <Box mb={4}>
