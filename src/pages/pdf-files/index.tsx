...
type ColumnType = ColumnDef<PdfFileInterface, unknown>;

interface PdfFileListPageProps {
...
export function PdfFileListPage(props: PdfFileListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPdfFiles({
        ...
        order: params.order,
        searchTermKeys: ['file_name.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PdfFileInterface>>(
    () => `/pdf-files?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/pdf-files/view/${record.id}`} passHref legacyBehavior>
                <Button
